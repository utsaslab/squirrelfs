...
impl inode::Operations for InodeOps {
    ...
    fn create(
        ...
        let (_new_dentry, new_inode) = squirrelfs_create(sbi, dir, dentry, umode, excl)?;
    ...
    fn link(old_dentry: &fs::DEntry, dir: &mut fs::INode, dentry: &fs::DEntry) -> Result<i32> {
        ...
        let result = squirrelfs_link(sbi, old_dentry, dir, dentry);
    ...
    fn mkdir(
        ...
        let (_new_dentry, _parent_inode, new_inode) = squirrelfs_mkdir(sbi, dir, dentry, umode)?;
    ...
    fn rmdir(dir: &mut fs::INode, dentry: &fs::DEntry) -> Result<i32> {
        ...
        let result = squirrelfs_rmdir(sbi, dir, dentry);
        match result {
            Ok(_) => Ok(0),
            Err(e) => Err(e),
        }
    ...
    fn rename(
        ...
        let result = squirrelfs_rename(sbi, old_dir, old_dentry, new_dir, new_dentry, flags);
    ...
    fn unlink(dir: &mut fs::INode, dentry: &fs::DEntry) -> Result<()> {
        ...
        let result = squirrelfs_unlink(sbi, dir, dentry);
        if let Err(e) = result {
            return Err(e);
        }
    ...
    fn symlink(
        ...
        let result = squirrelfs_symlink(sbi, dir, dentry, symname, mode);
        if let Err(e) = result {
            return Err(e);
...
// TODO: shouldn't really be generic but SquirrelFs isn't accessible here
/// squirrelfs_iget is for obtaining the VFS inode for an inode that already
/// exists persistently. new_vfs_inode is for setting up VFS inodes for
/// completely new inodes
pub(crate) fn squirrelfs_iget(
    ...
    match inode_type {
        InodeType::REG => unsafe {
            ...
            if let Some(pages) = pages {
                let inode_info = Box::try_new(SquirrelFsRegInodeInfo::new_from_tree(
                    ino,
                    pages,
                    pi.get_blocks(),
                ))?;
                ...
                let inode_info = Box::try_new(SquirrelFsRegInodeInfo::new(ino))?;
                (*inode).i_private = inode_info.into_foreign() as *mut _;
            }
        ...
        InodeType::DIR => unsafe {
            ...
            if let Some(pages) = pages {
                ...
                let inode_info = if let Some(mut dentries) = dentries {
                    ...
                    Box::try_new(SquirrelFsDirInodeInfo::new_from_tree(ino, pages, dentries))?
                } else {
                ...
                let dentries = new_dentry_tree(ino, parent_ino)?;
                let inode_info = Box::try_new(SquirrelFsDirInodeInfo::new(ino, dentries))?;
                (*inode).i_private = inode_info.into_foreign() as *mut _;
            }
...
// TODO: add type
/// new_vfs_inode is used to set up the VFS inode for a completely new SquirrelFsInode.
/// if the SquirrelFsInode already exists, you should use squirrelfs_iget
fn new_vfs_inode<'a, Type>(
    ...
    umode: bindings::umode_t,
    inode_info: Option<Box<SquirrelFsRegInodeInfo>>,
    parent_ino: InodeNum,
) -> Result<*mut bindings::inode> {
...
pub(crate) fn rmdir_delete_pages<'a>(
    sbi: &'a SbInfo,
    // delete_dir_info: &SquirrelFsDirInodeInfo,
...
fn squirrelfs_symlink<'a>(
    ...
    let pi_info = Box::try_new(SquirrelFsRegInodeInfo::new(pi.get_ino())?)?;
    ...
    let (_, pages) = pages.zero_pages(sbi, SQUIRRELFS_PAGESIZE, 0)?;
...
fn squirrelfs_truncate<'a>(
    ...
    if pi_size > new_size {
        ...
        if new_size % SQUIRRELFS_PAGESIZE == pi_size % SQUIRRELFS_PAGESIZE {
            Ok(())
        } else {
            let pages =
        ...
        let (bytes_written, pages) = match pages {
            ...
            Err(pages) => {
                ...
                let new_page_bytes = if pi_size % SQUIRRELFS_PAGESIZE != 0 {
                    // if the end of the file isn't page aligned, we don't
                    // count bytes at the end of the last page
                    let bytes_at_end = SQUIRRELFS_PAGESIZE - (pi_size % SQUIRRELFS_PAGESIZE);
                    alloc_offset += bytes_at_end;
                    bytes_to_truncate - bytes_at_end
                } else {
                    bytes_to_truncate
                };
                let pages_to_truncate = if new_page_bytes % SQUIRRELFS_PAGESIZE == 0 {
                    new_page_bytes / SQUIRRELFS_PAGESIZE
                } else {
                    (new_page_bytes / SQUIRRELFS_PAGESIZE) + 1 // to account for division rounding down
                };
                let pages = pages
