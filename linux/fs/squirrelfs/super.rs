...
#[vtable]
impl fs::Context<Self> for SquirrelFs {
...
impl fs::Type for SquirrelFs {
    type Context = Self;
    type Data = Box<SbInfo>;
    type InodeOps = InodeOps;
    type DirOps = DirOps;
    const SUPER_TYPE: fs::Super = fs::Super::BlockDev;
    const NAME: &'static CStr = c_str!("squirrelfs");
    ...
    fn fill_super(
        ...
        let sb = if let Some(true) = data.mount_opts.init {
            ...
            // let inode_info = Box::try_new(SquirrelFsDirInodeInfo::new(ROOT_INO))?;
    ...
    fn dirty_inode(inode: &fs::INode, _flags: i32) {
        ...
        squirrelfs_flush_buffer(raw_pi, core::mem::size_of::<SquirrelFsInode>(), true);
    }
            ...
            || unsafe { bindings::S_ISLNK(mode.try_into().unwrap()) }
        {
            ...
            if link_count == 0 {
                ...
                let inode_info = unsafe {
                    <Box<SquirrelFsRegInodeInfo> as ForeignOwnable>::from_foreign(
            ...
            if sbi.inodes_to_free.check_and_remove(ino) {
                ...
                // using from_foreign should make sure the info structure is dropped here
                let inode_info = unsafe {
                    <Box<SquirrelFsDirInodeInfo> as ForeignOwnable>::from_foreign(
                        (*inode.get_inner()).i_private,
                    )
                };
                unsafe { (*inode.get_inner()).i_private = core::ptr::null_mut() };
    ...
    fn init_private(inode: *mut bindings::inode) -> Result<()> {
        ...
        let inode_info = Box::try_new(SquirrelFsDirInodeInfo::new(ROOT_INO, dentries))?;
...
unsafe fn init_fs<T: fs::Type + ?Sized>(
    ...
    unsafe {
        let data_page_start = sbi.get_data_pages_start_page() * SQUIRRELFS_PAGESIZE;
        memset_nt(
...
fn remount_fs(sbi: &mut SbInfo) -> Result<()> {
    ...
    squirrelfs_flush_buffer(sb, SB_SIZE.try_into()?, true);
    Ok(())
}
...
pub(crate) trait PmDevice {
    fn get_pm_info(&mut self, sb: &fs::NewSuperBlock<'_, SquirrelFs>) -> Result<()>;
}
    ...
    fn get_pm_info(&mut self, sb: &fs::NewSuperBlock<'_, SquirrelFs>) -> Result<()> {
        ...
        let num_blocks = unsafe {
            bindings::dax_direct_access(
                dax_dev,
                0,
                (u64::MAX / SQUIRRELFS_PAGESIZE).try_into()?,
        ...
        let pgsize_i64: i64 = SQUIRRELFS_PAGESIZE.try_into()?;
        ...
        let bytes_per_inode = pages_per_inode * SQUIRRELFS_PAGESIZE;
        pr_info!("device size: {:?}\n", device_size);
        let num_inodes: u64 = device_size / bytes_per_inode;
        let inode_table_size = num_inodes * INODE_SIZE;
        let inode_table_pages = (inode_table_size / SQUIRRELFS_PAGESIZE) + 1; // account for possible rounding down
        let num_pages = num_inodes * pages_per_inode;
        let page_desc_table_size = num_pages * PAGE_DESCRIPTOR_SIZE;
        let page_desc_table_pages = (page_desc_table_size / SQUIRRELFS_PAGESIZE) + 1;
        pr_info!(
