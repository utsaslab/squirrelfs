...
#[allow(dead_code)]
#[inline(never)]
pub(crate) extern "C" fn squirrelfs_flush_buffer<T: ?Sized>(ptr: *const T, len: usize, fence: bool) {
    // #[cfg(CONFIG_SQUIRRELFS_DEBUG)]
    unsafe {
        bindings::flush_buffer(ptr as *const c_void, len.try_into().unwrap(), fence);
    }
    // // #[cfg(not(feature = "no_persist"))]
    // #[cfg(not(CONFIG_SQUIRRELFS_DEBUG))]
    // {
...
pub(crate) unsafe fn flush_edge_cachelines(ptr: *mut c_void, size: u64) -> Result<()> {
    ...
    if raw_ptr & 0x7 != 0 {
        squirrelfs_flush_buffer(ptr, 1, false);
    }
    if (raw_ptr + size) & 0x7 != 0 {
        unsafe { squirrelfs_flush_buffer(ptr.offset(size.try_into()?), 1, false) };
    }
