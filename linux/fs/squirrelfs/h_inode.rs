...
#[repr(C)]
pub(crate) struct SquirrelFsInode {
...
impl core::fmt::Debug for SquirrelFsInode {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("SquirrelFsInode")
...
impl SquirrelFsInode {
    /// Unsafe inode constructor for temporary use with init_fs only
    /// Does not flush the root inode
    pub(crate) unsafe fn init_root_inode(sbi: &SbInfo, inode: *mut bindings::inode) -> Result<&SquirrelFsInode> {
...
impl<'a, State, Op> InodeWrapper<'a, State, Op, RegInode> {
    ...
    pub(crate) fn get_inode_info(&self) -> Result<&SquirrelFsRegInodeInfo> {
        match self.vfs_inode {
            Some(vfs_inode) => unsafe {Ok(<Box::<SquirrelFsRegInodeInfo> as ForeignOwnable>::borrow((*vfs_inode).i_private))},
            None => {pr_info!("ERROR: inode {:?} is uninitialized\n", self.ino); Err(EPERM)}
        }
    }
}
...
impl<'a, State, Op: Initialized> InodeWrapper<'a, State, Op, DirInode> {
    // TODO: safety
    pub(crate) fn get_inode_info(&self) -> Result<&SquirrelFsDirInodeInfo> {
        match self.vfs_inode {
            Some(vfs_inode) => unsafe {Ok(<Box::<SquirrelFsDirInodeInfo> as ForeignOwnable>::borrow((*vfs_inode).i_private))},
...
impl<'a, Op, Type> InodeWrapper<'a, Dirty, Op, Type> {
    pub(crate) fn flush(self) -> InodeWrapper<'a, InFlight, Op, Type> {
        squirrelfs_flush_buffer(self.inode, mem::size_of::<SquirrelFsInode>(), false);
        Self::new(self)
    }
}
...
pub(crate) struct BitmapInodeAllocator {
...
impl InodeAllocator for BitmapInodeAllocator {
    ...
    fn alloc_ino(&self, sbi: &SbInfo) -> Result<InodeNum> {
        ...
        let mut counter: usize = usize::MAX;
        for (index, inode) in bitmap.iter().enumerate() {
            if *inode == false {
                counter = index.try_into()?;
                break;
            }
        }
        if counter >= 0 {
            ...
            return Ok(counter as InodeNum);
        }
        pr_info!("ERROR: ran out of inodes in bitmap inode allocator\n");
        Err(ENOSPC)
    }
    ...
    fn dealloc_ino(&self, ino: InodeNum, sbi: &SbInfo) -> Result<()> {
        let bitmap = Arc::clone(&self.bitmap);
        let mut bitmap = bitmap.lock();
        if ino > bitmap.len().try_into()? {
            pr_info!("ERROR: inode {:?} is out of bounds\n", ino);
            return Err(EINVAL);
        }
        if bitmap[ino as usize] == false {
            pr_info!("ERROR: inode {:?} was deallocated but is already in allocator\n", ino);
            return Err(EINVAL);
        }
        bitmap[ino as usize] = false;
