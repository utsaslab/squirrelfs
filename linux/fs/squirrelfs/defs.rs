...
pub(crate) const SQUIRRELFS_PAGESIZE: u64 = 4096;
...
pub(crate) const SB_SIZE: u64 = SQUIRRELFS_PAGESIZE;
...
#[repr(C)]
pub(crate) struct SquirrelFsSuperBlock {
    magic: i64,
    block_size: u64,
    size: i64,
    clean_unmount: bool,
...
pub(crate) struct SbInfo {
    ...
    // impl depends on SquirrelFs definition and fs::Type impl, which logically
    ...
    pub(crate) mount_opts: SquirrelFsParams,
...
impl SbInfo {
    ...
    pub(crate) fn new() -> Self {
        SbInfo {
            ...
            mode: 0o755,
            blocksize: SQUIRRELFS_PAGESIZE.try_into().unwrap(),
            num_blocks: 0,
            ...
            mount_opts: SquirrelFsParams::default(),
            num_inodes: 0,
            inode_table_size: 0,
            inode_table_pages: 0,
            num_pages: 0,
    ...
    pub(crate) fn get_super_block(&mut self) -> Result<&SquirrelFsSuperBlock> {
        let super_block = unsafe { &*(self.virt_addr as *mut SquirrelFsSuperBlock) };
    ...
    pub(crate) fn get_super_block_mut(&self) -> Result<&mut SquirrelFsSuperBlock> {
        let super_block = unsafe { &mut *(self.virt_addr as *mut SquirrelFsSuperBlock) };
        // assume for now that if the magic is fine, the rest of the super block is fine
        if super_block.magic != SUPER_MAGIC {
            pr_err!(
                "Magic should be {:?} but found {:?}\n",
    ...
    pub(crate) fn get_page_desc_table<'a>(&self) -> Result<&'a mut [PageDescriptor]> {
        let page_desc_table_addr = unsafe {
            self.virt_addr
                .offset((SQUIRRELFS_PAGESIZE * self.get_page_desc_table_start_page()).try_into()?)
    ...
    pub(crate) fn get_inode_table<'a>(&self) -> Result<&'a mut [SquirrelFsInode]> {
        let inode_table_addr: *mut SquirrelFsInode = unsafe {
            self.virt_addr
                .offset((SQUIRRELFS_PAGESIZE * self.get_inode_table_start_page()).try_into()?)
                as *mut SquirrelFsInode
        };
        let table =
    ...
    pub(crate) fn get_inode_by_ino<'a>(&self, ino: InodeNum) -> Result<&'a SquirrelFsInode> {
