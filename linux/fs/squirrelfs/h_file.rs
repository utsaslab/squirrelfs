...
impl file::Operations for FileOps {
    ...
    fn fsync(_data: (), file: &file::File, start: u64, end: u64, _datasync: bool) -> Result<u32> {
        if unsafe { bindings::mapping_mapped((*file.get_inner()).f_mapping) != 0 } {
            ...
            let result = squirrelfs_msync(sbi, pi_info, start, len);
            match result {
                Ok(_) => Ok(0),
                Err(e) => Err(e),
            }
    ...
    fn write(
        ...
        let result = squirrelfs_write(sbi, inode, reader, offset);
    ...
    fn read(
        ...
        let result = squirrelfs_read(sbi, file, inode, writer, offset);
    ...
    fn fallocate(
        ...
        return squirrelfs_fallocate(inode, sbi, data, file, mode, offset, len);
    }
...
fn squirrelfs_fallocate(
    ...
    if falloc_fl_collapse_range && 
        ...
        (offset_u64 % SQUIRRELFS_PAGESIZE != 0 || len_u64 % SQUIRRELFS_PAGESIZE != 0) // Treat pages as blocks?
    { 
        return Err(EINVAL);
    }
    ...
    if falloc_fl_insert_range && 
        ...
        (offset_u64 % SQUIRRELFS_PAGESIZE != 0 || len_u64 % SQUIRRELFS_PAGESIZE != 0)
    { 
        return Err(EINVAL);
    }
    ...
    */
    ...
    /*
     * Implementation of FALLOC_FL_COLLAPSE_RANGE
     * 
     * Collapses a range (offset -> offset + len) and removes it. Deallocating the pages present in the range
     * and move the pages past that range to the front.
     */
    if falloc_fl_collapse_range {
...
fn squirrelfs_write<'a>(
    ...
    match sbi.mount_opts.write_type {
        ...
        Some(WriteType::SinglePage) => {
            let count = if SQUIRRELFS_PAGESIZE < len {
                SQUIRRELFS_PAGESIZE
            } else {
                len
            };
...
fn single_page_write<'a>(
    ...
    let bytes_after_offset = SQUIRRELFS_PAGESIZE - offset_in_page;
...
fn runtime_checked_write<'a>(
    ...
    while bytes < len {
        ...
        bytes += SQUIRRELFS_PAGESIZE;
        loop_offset = page_offset + SQUIRRELFS_PAGESIZE;
    }
    ...
    for page in pages.drain(..) {
        ...
        let bytes_to_write = if len < SQUIRRELFS_PAGESIZE - offset_in_page {
            len
        } else {
            SQUIRRELFS_PAGESIZE - offset_in_page
        };
        let (written, page) = page.write_to_page(sbi, reader, offset_in_page, bytes_to_write)?;
        bytes_written += written;
        page_offset += SQUIRRELFS_PAGESIZE;
        len -= bytes_to_write;
        offset_in_page = 0;
    ...
    let page_list = match page_list {
        ...
        Err(page_list) => {
            ...
            let allocation_offset =
                page_offset(alloc_offset)? + page_list.len() * SQUIRRELFS_PAGESIZE;
...
fn squirrelfs_read(
    ...
    while count > 0 {
        ...
        let offset_in_page = offset - page_offset;
        let bytes_left_in_page = SQUIRRELFS_PAGESIZE - offset_in_page;
...
impl iomap::Operations for IomapOps {
    fn iomap_begin(
        ...
        let (num_pages, addr) =
            squirrelfs_iomap_get_blocks(sbi, inode, first_block << blkbits, length, create)?;
        ...
        if num_pages == 0 {
            ...
            unsafe {
                ...
                (*iomap).length = (num_pages * SQUIRRELFS_PAGESIZE).try_into()?;
...
fn squirrelfs_iomap_get_blocks(
    sbi: &SbInfo,
    inode: &fs::INode,
    page_offset: i64,
    length: i64,
    ...
    let pagesize_i64: i64 = SQUIRRELFS_PAGESIZE.try_into()?;
...
fn get_num_pages_in_region(count: u64, offset: u64) -> u64 {
    if offset % SQUIRRELFS_PAGESIZE == 0 {
        if count % SQUIRRELFS_PAGESIZE == 0 {
            count / SQUIRRELFS_PAGESIZE
        } else {
            (count / SQUIRRELFS_PAGESIZE) + 1
        }
    } else {
        let first_page_bytes = SQUIRRELFS_PAGESIZE - (offset % SQUIRRELFS_PAGESIZE);
        let remaining_bytes = if count < first_page_bytes {
            count
        } else {
            count - first_page_bytes
        };
        if remaining_bytes % SQUIRRELFS_PAGESIZE == 0 {
            (remaining_bytes / SQUIRRELFS_PAGESIZE) + 1 // +1 for the first page
        } else {
            (remaining_bytes / SQUIRRELFS_PAGESIZE) + 2 // +2 for the first and last page
        }
    }
}
