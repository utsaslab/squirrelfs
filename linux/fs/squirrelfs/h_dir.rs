...
#[repr(C)]
#[derive(Debug)]
pub(crate) struct SquirrelFsDentry {
    is_dir: u16,
...
impl SquirrelFsDentry {
    // Getters are not unsafe; only modifying SquirrelFsDentry is unsafe
...
impl<'a, State, Op> DentryWrapper<'a, State, Op> {
    ...
    pub(crate) fn get_dentry_offset(&self, sbi: &'a SbInfo) -> u64 {
        let dentry_virtual_addr = self.dentry as *const SquirrelFsDentry as u64;
...
impl<'a> DentryWrapper<'a, Clean, Recovering> {
    pub(crate) fn from_dinfo(info: DentryInfo) -> Result<Self> {
        ...
        // persistent dentry
        let dentry: &mut SquirrelFsDentry =
            unsafe { &mut *(info.get_virt_addr().unwrap() as *mut SquirrelFsDentry) };
        Ok(Self {
            state: PhantomData,
            op: PhantomData,
            dentry,
    ...
    pub(crate) fn rename_ptr(&self, sbi: &SbInfo) -> Result<Option<&'a mut SquirrelFsDentry>> {
        if self.dentry.rename_ptr == 0 {
            ...
            let pg_sz: isize = SQUIRRELFS_PAGESIZE.try_into().unwrap();
            ...
            if off < start || off >= end {
                ...
                let ptr = unsafe { base.offset(off) } as *mut SquirrelFsDentry;
...
impl<'a> DentryWrapper<'a, Clean, Free> {
    ...
    pub(crate) unsafe fn wrap_free_dentry(dentry: &'a mut SquirrelFsDentry) -> Self {
        Self {
            state: PhantomData,
            op: PhantomData,
            dentry: dentry,
...
impl<'a> DentryWrapper<'a, Clean, Start> {
    pub(crate) fn get_init_dentry(info: DentryInfo) -> Result<Self> {
        ...
        match info.get_virt_addr() {
            Some(virt_addr) => {
                let dentry: &mut SquirrelFsDentry =
                    unsafe { &mut *(virt_addr as *mut SquirrelFsDentry) };
...
impl<'a> DentryWrapper<'a, Clean, SetRenamePointer> {
    ...
    pub(crate) fn init_rename_pointer_dir_regular(
        ...
        // dst_parent_inode_info: &SquirrelFsDirInodeInfo, // TODO: obtain in the fxn to make sure we use the right one
    ) -> Result<(
        DentryWrapper<'a, Clean, Renamed>,
        DentryWrapper<'a, Dirty, InitRenamePointer>,
    )> {
...
impl<'a> DentryWrapper<'a, Clean, Complete> {
    ...
    pub(crate) fn index(&self, parent_inode_info: &SquirrelFsDirInodeInfo) -> Result<()> {
...
impl<'a, Op> DentryWrapper<'a, Dirty, Op> {
    pub(crate) fn flush(self) -> DentryWrapper<'a, InFlight, Op> {
        squirrelfs_flush_buffer(self.dentry, mem::size_of::<SquirrelFsDentry>(), false);
        DentryWrapper {
            state: PhantomData,
            op: PhantomData,
            dentry: self.dentry,
...
impl dir::Operations for DirOps {
    fn iterate(f: &file::File, ctx: *mut bindings::dir_context) -> Result<u32> {
        ...
        let result = squirrelfs_readdir(sbi, inode, ctx);
        match result {
            Ok(r) => Ok(r),
            Err(e) => Err(e),
        }
...
pub(crate) fn squirrelfs_readdir(
    sbi: &SbInfo,
    dir: &mut fs::INode,
