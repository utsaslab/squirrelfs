...
impl PageAllocator for Option<PerCpuPageAllocatorBitmap> {
    ...
    fn new_from_alloc_vec(
        ...
        for _i in 0..cpus_u64 { // setup the bitmaps empty
            // let mut bitmap = vec![false; pages_per_cpu as usize];
            let mut bitmap = Vec::try_with_capacity(pages_per_cpu as usize)?;
            bitmap.try_resize(pages_per_cpu as usize, false)?;
...
#[derive(Debug)]
struct DirPage<'a> {
    dentries: &'a mut [SquirrelFsDentry],
}
...
impl DirPage<'_> {
    pub(crate) fn get_dentry_info_from_dentries(self) -> Result<Vec<DentryInfo>> {
        ...
        for d in self.dentries.iter() {
            ...
            if !d.is_free() {
                let name = d.get_name();
                let virt_addr = d as *const SquirrelFsDentry as *const ffi::c_void;
...
impl<'a> DirPageWrapper<'a, Clean, Start> {
    ...
    pub(crate) fn from_dentry<State, Op>(
        ...
        let page_no = dentry_offset / SQUIRRELFS_PAGESIZE;
...
unsafe fn page_no_to_page(sbi: &SbInfo, page_no: PageNum) -> Result<*mut u8> {
    if page_no > MAX_PAGES {
        ...
        let res = Ok(unsafe { virt_addr.offset((SQUIRRELFS_PAGESIZE * page_no).try_into()?) });
        res
    }
}
...
impl<'a, Op> DirPageWrapper<'a, Dirty, Op> {
    pub(crate) fn flush(mut self) -> DirPageWrapper<'a, InFlight, Op> {
        match &self.page.page {
            ...
            Some(page) => squirrelfs_flush_buffer(*page, mem::size_of::<DirPageHeader>(), false),
    ...
    pub(crate) fn unmap(self, sbi: &SbInfo) -> Result<DirPageListWrapper<InFlight, ClearIno>> {
        ...
        while page.is_some() {
            if let Some(page) = page {
                ...
                ph.ino = 0;
                squirrelfs_flush_buffer(ph, mem::size_of::<DirPageHeader>(), false);
            }
            pages.move_next();
            page = pages.current();
        }
...
pub(crate) fn page_offset(offset: u64) -> Result<u64> {
    // integer division removes the remainder; multiplying by SQUIRRELFS_PAGESIZE
    // gives us the offset of the page to read/write
    Ok((offset / SQUIRRELFS_PAGESIZE) * SQUIRRELFS_PAGESIZE)
}
...
impl<'a, Op> DataPageWrapper<'a, Dirty, Op> {
    pub(crate) fn flush(mut self) -> DataPageWrapper<'a, InFlight, Op> {
        match &self.page.page {
            Some(page) => squirrelfs_flush_buffer(*page, mem::size_of::<DataPageHeader>(), false),
            None => panic!("ERROR: Wrapper does not have a page"),
        };
...
impl<State, Op> DataPageListWrapper<State, Op> {
    ...
    pub(crate) fn first_page_virt_addr(&self) -> Option<*mut ffi::c_void> {
        ...
        if let Some(page) = page {
            let page_no = page.get_page_no();
            let addr = page_no * SQUIRRELFS_PAGESIZE;
    ...
    pub(crate) fn get_data_page_list(
        ...
        while bytes < len {
            ...
            if offset % SQUIRRELFS_PAGESIZE == 0 {
                ...
                offset = page_offset + SQUIRRELFS_PAGESIZE;
            } else {
                let bytes_in_page = SQUIRRELFS_PAGESIZE - (offset % SQUIRRELFS_PAGESIZE);
                bytes += bytes_in_page;
        ...
        while page.is_some() {
            if let Some(page) = page {
                ...
                if get_offset_of_page_no(sbi, page_no)? < page_offset {
                    ...
                    let bytes_to_write = if len < SQUIRRELFS_PAGESIZE - offset_within_page {
                        len
                    } else {
                        SQUIRRELFS_PAGESIZE - offset_within_page
                    };
                    // let bytes_to_write =
    ...
    pub(crate) fn msync_pages(self, sbi: &SbInfo) -> Result<DataPageListWrapper<Clean, Msynced>> {
        ...
        while page.is_some() {
            if let Some(page) = page {
                let page_no = page.get_page_no();
                let ptr = unsafe { page_no_to_page(sbi, page_no)? };
                squirrelfs_flush_buffer(ptr, SQUIRRELFS_PAGESIZE.try_into()?, false);
            } else {
                unreachable!();
            }
            page_list.move_next();
...
impl DataPageListWrapper<Clean, ToUnmap> {
    pub(crate) fn get_data_pages_to_truncate<'a>(
        ...
        if first_page_offset % SQUIRRELFS_PAGESIZE == 0 {
            ...
            pages.push_back(Box::try_new(LinkedPage::new(data_page_no))?);
            offset = first_page_offset + SQUIRRELFS_PAGESIZE;
            bytes += SQUIRRELFS_PAGESIZE;
            num_pages += 1;
        } else {
            let bytes_at_end = SQUIRRELFS_PAGESIZE - (first_page_offset % SQUIRRELFS_PAGESIZE);
            offset += bytes_at_end;
            bytes += bytes_at_end;
        }
