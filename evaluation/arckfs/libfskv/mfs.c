...
s64 sufs_libfs_readm_kv(struct sufs_libfs_mnode *m, char *buf, u64 nbytes)
{
    u64 end = sufs_libfs_mnode_file_size(m);
    ...
    addr = sufs_libfs_mnode_file_get_page(m, 0);

    if (!addr)
        goto out;

#if 0
    printf("read: buf %lx, addr: %lx, end: %ld, nbytes: %ld\n", (unsigned long) buf, addr, end, nbytes);
#endif
    memcpy(buf, (char*) addr, end);
...
s64 sufs_libfs_writem_kv(struct sufs_libfs_mnode *m, char *buf, u64 nbytes)
{
    u64 end = sufs_libfs_mnode_file_size(m);
    u64 off = 0;
    unsigned long addr = 0;
    ...
    if (!addr)
    {
        ...
        sufs_libfs_mnode_file_resize_append(m, SUFS_FILE_BLOCK_SIZE, addr);
    }

#if 0
    printf("write: buf %lx, addr: %lx, end: %ld, nbytes: %ld\n", (unsigned long) buf, addr, end, nbytes);
#endif

    memcpy((char*) addr, buf, end);
