        ...
        char *new_path)
{
    ...
    /* BUG: Need to enable this once we have the scalable readers-writer lock */
#if BUG
            ...
            mfold, mfold_type == SUFS_FILE_TYPE_DIR ? mfold : NULL, &item))
    {
        ...
        /*
         * BUG: Need to enable journal once we have put
         * journal in the corresponding
         * NUMA node
         */
...
unsigned long sufs_libfs_get(struct sufs_libfs_proc * proc, char * path,
        void * buf, unsigned long count)
{
    ...
    return sufs_libfs_readm_kv(m, buf, count);
}


unsigned long sufs_libfs_set(struct sufs_libfs_proc * proc, char * path,
        void * buf, unsigned long count)
{
    struct sufs_libfs_mnode *m = NULL;
    int err = 0;

    m = sufs_libfs_create(proc->cwd_m, path, SUFS_FILE_TYPE_REG,
            0666, proc->uid, proc->gid, false, &err);
    ...
    return sufs_libfs_writem_kv(m, buf, count);
}
