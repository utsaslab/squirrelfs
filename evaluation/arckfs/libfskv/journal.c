...
void sufs_libfs_lite_journal_hard_init(struct sufs_libfs_super_block *sb)
{
     ...
     */
    orig_nums = (PAGE_ROUND_UP(cpus * SUFS_CACHELINE) / SUFS_PAGE_SIZE) + cpus;
    ...
    for (i = 0; i < cpus; i++)
    {
        ...
        unsigned long content_addr = sb->journal_addr +
                PAGE_ROUND_UP(cpus * SUFS_CACHELINE) + i * SUFS_PAGE_SIZE;
