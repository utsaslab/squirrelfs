        ...
        int ino_num, int parent_mnum, struct sufs_inode *inode)
{
    ...
    m->node = sufs_libfs_current_node();

    /* m->node = sufs_libfs_xor_random() % (sufs_libfs_sb.pm_nodes); */
...
sufs_libfs_mnode_dir_build_index_one_file_block(struct sufs_libfs_mnode * mnode,
        unsigned long offset, char * prefix)
{
...
#if 0
            printf("insert during index building with prefix: %s, dir->name: %s, and path: %s\n",
                    prefix, dir->name, path);
#endif

            if (sufs_libfs_chainhash_insert(&sufs_libfs_dir_map, path,
                    PATH_MAX, dir->ino_num, (unsigned long) dir, NULL) == false)
                return -ENOMEM;
        }
...
#endif
            sufs_libfs_mnode_dir_build_index_one_file_block(mnode,
                    idx->offset, prefix);
            idx++;
        }
        else
...
struct sufs_libfs_mnode*
sufs_libfs_mnode_dir_lookup(struct sufs_libfs_mnode *mnode, char * path)
{
    unsigned long ino = 0;
    ...
    if (sufs_libfs_map_file(mnode, 0, path) != 0)
    {
#if 0
    ...
    sufs_libfs_chainhash_lookup(&sufs_libfs_dir_map, path, PATH_MAX,
            &ino, NULL);
...
bool sufs_libfs_mnode_dir_insert(struct sufs_libfs_mnode *mnode, char *name,
        int name_len, char * path, struct sufs_libfs_mnode *mf,
        struct sufs_dir_entry **dirp)
{
    bool ret = false;
    struct sufs_dir_entry *dir = NULL;
    ...
    if (sufs_libfs_map_file(mnode, 1, path) != 0)
    {
#if 0
        printf("map failed!\n");
        fflush(stdout);
...
bool sufs_libfs_mnode_dir_remove(struct sufs_libfs_mnode *mnode, char * path)
{
    bool ret = false;
    ...
    if (!sufs_libfs_chainhash_remove(&sufs_libfs_dir_map, path,
            PATH_MAX, NULL, (unsigned long*) &dir))
    {
        goto out;
    }
...
struct sufs_libfs_mnode*
sufs_libfs_mnode_dir_exists(struct sufs_libfs_mnode *mnode, char * path)
{
    unsigned long ret = 0;
...
int sufs_libfs_mnode_stat(struct sufs_libfs_mnode *m, struct stat *st)
{
    ...
    if (sufs_libfs_map_file(parent, 0, NULL) != 0)
        goto out;
