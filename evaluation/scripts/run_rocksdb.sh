...
load_workload() {
    ...
    time sudo -E numactl --membind=0 $HOME/ycsb-0.17.0/bin/ycsb load rocksdb -threads $numthreads -P "$HOME/ycsb-0.17.0/workloads/workload$workload_id" -p rocksdb.dir=/mnt/pmem/rocksdb -p recordcount=$record_count -p operationcount=$operation_count > $output_dir/$FS/rocksdb/Load$workload_id/Run$i 2>&1
...
run_workload() {
    ...
    time sudo -E numactl --membind=0 $HOME/ycsb-0.17.0/bin/ycsb run rocksdb -threads $numthreads -P "$HOME/ycsb-0.17.0/workloads/workload$workload_id" -p rocksdb.dir=/mnt/pmem/rocksdb -p recordcount=$record_count -p operationcount=$operation_count > $output_dir/$FS/rocksdb/Run$workload_id/Run$i 2>&1
